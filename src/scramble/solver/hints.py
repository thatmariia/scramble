...
def add_hints(mdl: CpModel, mv: ModelVariables):
    ...
    while team_idx < len(team_ids_with_players):
        ...
        for _ in range(min_teams_in_match):
    ...
    # add hints to the model
    for team_id in team_ids_with_players:
        mdl.add_hint(mv.team_active[team_id], 1)
    for court_id in court_ids_with_teams:
        mdl.add_hint(mv.court_active[court_id], 1)
    for (player_id, team_id), value in player_in_team.items():
        mdl.add_hint(mv.player_in_team[(player_id, team_id)], value)
    for (team_id, court_id), value in team_on_court.items():
        mdl.add_hint(mv.team_on_court[(team_id, court_id)], value)
